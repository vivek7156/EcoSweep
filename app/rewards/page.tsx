...
import { HandCoins, ArrowUpRight, ArrowDownRight, Gift, AlertCircle, Loader } from "lucide-react";
...
type Transaction = {
    id: number;
    type: 'earned_collect' | 'earned_report' | 'redeemed';
    amount: number;
        ...
        try{
            ...
            if(userEmail){
                ...
                if(fetchedUser){
                    ...
                    const calculateBalance = fetchedTransactions?.reduce((acc: any, transaction) => {
                        return transaction.type.startsWith('earned') ? acc + transaction.amount : acc - transaction.amount;
                    }, 0);
                    setBalance(Math.max(calculateBalance, 0));
                } else{
                    toast.error("User not found. Please login again");
                }
            }
        } catch(e){
            console.error("Error fetching user data and rewards", e);
          ...
          <div className="bg-white p-6 rounded-xl shadow-lg flex flex-col justify-between h-full border-l-4 border-green-500 mb-8">
            ...
            <div className="flex items-center justify-between mt-auto">
              <div className="flex items-center">
                <HandCoins className="w-10 h-10 mr-3 text-green-500" />
                <div>
