    ...
    import { Trash, MapPin, CheckCircle, Clock, Upload, Loader, Calendar, Weight, Search } from "lucide-react";
    import { Button } from "@/components/ui/button";
    import { Input } from "@/components/ui/input";
    import toast from "react-hot-toast";
    import { GoogleGenerativeAI } from "@google/generative-ai";
    import { createNotifications, getUserByEmail, getWasteCollectionTasks, saveCollectedWaste, saveReward, updateTaskStatus } from "@/utils/db/actions";
    ...
    export default function CollectPage() {
        ...
        useEffect(() => {
         const fetchUserAndTask = async () => {
            ...
            try{
                ...
                const fetchedTask = await getWasteCollectionTasks();
                setTasks(fetchedTask.map(task => ({
                  ...task,
                  status: task.status as "pending" | "in_progress" | "completed" | "verified"
                })));
            } catch(e){
                console.error("Error fetching user and Tasks", e);
                toast.error("Failed to load user and task. Please try again");
            } finally {
                setLoading(false);
            }
         };
        ...
        const handleStatusChange = async (taskId: number, newStatus: CollectionTask['status']) => {
            ...
            try {
              ...
              if(updatedTask){
                setTasks(tasks.map((task) => task.id === taskId ? {...task, status: newStatus, collectorId: user.id} : task));
        ...
        const handleVerify = async () => {
            ...
            try{
                ...
                try{
                    ...
                    await createNotifications(user.id, `You've earned ${earnedReward} points for collecting waste!`, "reward" );
        ...
        const pageCount = Math.ceil(filterTasks.length / ITEMS_PER_PAGE);
        ...
        return (
            <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto text-gray-600">
...
function StatusBadge({ status }: { status: CollectionTask['status'] }) {
    const statusConfig = {
      ...
      in_progress: { color: 'bg-blue-100 text-blue-800', icon: Trash },
